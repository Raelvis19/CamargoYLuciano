...
function Recetas() {
  ...
  const [resultadosMedicamentos, setResultadosMedicamentos] = useState([]);
  ...
  useEffect(() => {
    async function cargarInventario() {
      try {
        ...
        notify.error("No fue posible cargar los medicamentos del inventario.");
      } finally {
  ...
  useEffect(() => {
    function handleClickFuera(event) {
      ...
      if (
        ...
        !medicamentoRef.current.contains(event.target)
      ) {
  ...
  async function handleEliminarReceta(id) {
    ...
    try {
      ...
      console.error(err);
      notify.error("No se pudo eliminar la receta.");
    }
  }
  ...
  function handleBusquedaMedicamento(event) {
    const valor = event.target.value;

    setMedicamentoBuscado(valor);
    setMedicamentoSeleccionado(null);
    setMostrarMedicamentos(true);
    ...
    const coincidencias = inventario
      .filter((medicamento) => {
        const nombre = medicamento.nombre?.toLowerCase() || "";
        const codigo = String(medicamento.codigo ?? "").toLowerCase();

        return nombre.includes(texto) || codigo.includes(texto);
      })
      .slice(0, 8);

    setResultadosMedicamentos(coincidencias);
  }

  function seleccionarMedicamento(medicamento) {
    setMedicamentoSeleccionado(medicamento);
    setMedicamentoBuscado(medicamento.nombre);
    setResultadosMedicamentos([]);
    setMostrarMedicamentos(false);
  }
  function agregarMedicamento() {
    if (!medicamentoSeleccionado) {
      notify.warning(
        "Seleccione un medicamento existente en el inventario."
      );
      return;
    }

    if (!dosis.trim()) {
      notify.warning("Indique la dosis del medicamento.");
      return;
    }

    if (Number(medicamentoSeleccionado.cantidad) <= 0) {
      notify.warning(
        `${medicamentoSeleccionado.nombre} no tiene existencias disponibles.`
      );
      return;
    }

    const yaAgregado = medicamentosReceta.some(
      (medicamento) =>
        medicamento.codigo === medicamentoSeleccionado.codigo
    );

    if (yaAgregado) {
      notify.warning(
        "Este medicamento ya fue agregado a la receta."
      );
      return;
    }

    setMedicamentosReceta((anteriores) => [
      ...anteriores,
      {
        inventario_id: medicamentoSeleccionado.id,
    ...
    notify.success("Medicamento agregado a la receta.");
  }
  ...
  async function imprimirReceta() {
    if (!pacienteSeleccionado) {
      notify.warning("Seleccione un paciente.");
      return;
    }

    if (medicamentosReceta.length === 0) {
      notify.warning("Agregue al menos un medicamento a la receta.");
      return;
    }
    ...
    try {
      ...
      console.error(err);
      notify.error("No se pudo guardar la receta.");
    } finally {
      setGuardando(false);
    }
  }
              ...
              >
                ...
                <div className="input-group">
                  ...
                  <input
                    ...
                    onChange={handleBusquedaMedicamento}
                    onFocus={() => {
                      if (medicamentoBuscado.trim()) {
                        setMostrarMedicamentos(true);
                      }
                    }}
                    autoComplete="off"
                ...
                {mostrarMedicamentos && medicamentoBuscado.trim() && (
                  <div
                  ...
                  >
                    {resultadosMedicamentos.length === 0 ? (
                      ...
                      resultadosMedicamentos.map((medicamento) => (
                        <button
                          key={medicamento.id}
                          type="button"
                        ...
                        >
                          <div className="d-flex justify-content-between align-items-center gap-3">
                            ...
                            <span
                              className={`badge ${Number(medicamento.cantidad) > 0
                                  ? "text-bg-success"
                                  : "text-bg-danger"
                                }`}
                            >
                ...
                {medicamentoSeleccionado && (
                  <small className="d-block mt-2 text-success">
                    Seleccionado: {medicamentoSeleccionado.codigo} —{" "}
